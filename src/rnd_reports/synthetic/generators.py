"""Генераторы синтетических данных для R&D вокруг CUPED/CUPAC.

Портировано из репозитория VarWar (`data_gen.py`, класс ``DataGenerator``) без
изменения логики: контроль корреляционной структуры, два лага для Y, инструмент
``z``, скрытый конфаундер ``U``, механика акцепта ``D`` и потенциальные исходы
``y0/y1``. Добавлены типы и русские докстринги. Только синтетика, без реальных
данных. См. docs/migration_from_varwar.md.
"""
...
class DataGenerator:
    """Генератор данных с поддержкой двух лагов для Y и контролем корреляций."""

    def __init__(
        self,
        n_samples: int = 2000,
        distributions: Optional[dict] = None,
        time_correlations: Optional[dict] = None,
    ...
    def _generate_bernoulli_pair(self, p: float, rho: float):
    ...
    def _generate_correlated_pair(self, dist_type: str, params: dict, rho: float):
        """Генератор пар (текущее значение, лаг) с заданной корреляцией."""
        if dist_type == "normal":
            cov = [
    ...
    def _generate_correlated_chain(self, params: dict, rho: float, n_points: int):
        """Цепочка коррелированных значений (AR(1)) для нормального распределения."""
        mean = params["mean"]
        std = params["std"]
    ...
    def generate(self) -> pd.DataFrame:
        """Сгенерировать полный датасет с двумя лагами для Y."""
        data: dict = {}

        # Базовые переменные с лагами
        for var in ["X1", "X2"]:
        ...
        # Инструмент и скрытый конфаундер
