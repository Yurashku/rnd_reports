...
(`epk_id, report_dt, emb_{i}_val`; легаси `col_*` тоже распознаётся); трит — опциональная
колонка `treatment` того же датафрейма (при отсутствии генерируется случайно).

- **Статус:** 🔶 базовый эксперимент готов: тулкит-функции (pyspark) + causal-слой (numpy/sklearn) — оценка ATE с поправкой на эмбеддинги, диагностика баланса/overlap (синтетика, известный ATE).
