...
незначимости и какие метрики дают надёжный сигнал. На левой панели — **по одной** оценке эффекта
и её ДИ на каждую метрику: эффект (разница средних) — свойство метрики, а методы меняют только
порог значимости, не сам эффект. Цвет точек — по ДИ: красные не включают 0 (сырой сигнал, до
поправки на множественность), синие включают. Справа на каждой строке-метрике разнесены по
вертикали raw p и adjusted p всех методов — так видно, как именно каждая поправка двигает p-value.
