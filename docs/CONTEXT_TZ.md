...
> ⚠️ **Историзм.** Документ описывает раннее ТЗ, когда репозиторий состоял из одного
> RnD (`rnd_01_ab_дизайн_экспериментов`) с под-ноутбуками. Текущая структура иная:
> отчётные RnD лежат в `rnd/NN_<topic>/`, общий код — в `src/rnd_reports/`.
> **Актуальный источник правды по структуре — корневой [`README.md`](../README.md).**
> Пути и формулировки ниже сохранены как исторический контекст и могут не совпадать с репозиторием.

- **Название проекта:** `rnd_reports` — R&D-репозиторий по дизайну A/B-экспериментов
- **Версия документа:** v0.1
- **Дата:** 2026-02-26
- **Статус:** Draft (исторический)
