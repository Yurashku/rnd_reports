...
- `embeddings/` — тулкит-функции RnD-7 для pyspark-эмбеддингов (`epk_id, report_dt, emb_{i}_val`; легаси `col_*` распознаётся), каждая принимает один DataFrame и добавляет свои колонки: `reducer.py` (`reduce_embeddings`, StandardScaler+PCA-снижение → добавляет `red_*`), `propensity.py` (`add_propensity_score`, лучший по ROC-AUC из LogisticRegression/GBTClassifier → добавляет `prop_score`; трит — опциональная колонка `treatment`, при отсутствии генерируется случайно с `random_state`); `experiment.py` — реализованный causal-слой (numpy/sklearn): ATE с поправкой + диагностика баланса/overlap. pyspark — **опциональный extra** (`pip install -e .[spark]`); импорт пакета его не требует.
