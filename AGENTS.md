# AGENTS.md — инструкции для агентов в этом репозитории

> Этот файл синхронизирован с [`CLAUDE.md`](CLAUDE.md): общий блок «Карта репозитория»,
...
1. При любом изменении в репозитории обновляй `README.md` (и при необходимости `CLAUDE.md`/`AGENTS.md`), чтобы они оставались актуальными.
...
5. Для каждого RnD важно хранить: краткий отчёт, воспроизводимый и читаемый код симуляций (Jupyter), ссылки из отчёта на соответствующий код.
6. В конце любой задачи убеждайся в **чистоте** (нет мусорных файлов; артефакты только в `results/`; данные не в git) и **синхронизации** репозитория (`CLAUDE.md ↔ AGENTS.md` совпадают; `report.md`/`README`/`docs`/`configs`, вшитый код `notebook_internal.ipynb` и экспорты пакета соответствуют коду). Подробности — в [`docs/WORKFLOW.md`](docs/WORKFLOW.md) §«Чистота и синхронизация».
7. **Всегда** оформляй изменения коммитом и pull request'ом: заводи отдельную ветку (не коммить в `main`), коммить, пушь и открывай PR (`gh pr create`). Подробности — в [`docs/WORKFLOW.md`](docs/WORKFLOW.md) §«Git: коммиты и PR».
...
- `embeddings/` — RnD-7: тулкит-функции pyspark-эмбеддингов (`reducer.py` `reduce_embeddings`:
  PCA-снижение, добавляет `red_*`; `propensity.py` `add_propensity_score`: prop_score, выбор
  LogReg/GBT по ROC-AUC, авто-random трит при отсутствии колонки `treatment`); каждая функция
  принимает один DataFrame и добавляет свои колонки; pyspark — опциональный extra `[spark]`.
...
- В конце задачи репозиторий должен быть чист и синхронен (см. `docs/WORKFLOW.md`
  §«Чистота и синхронизация»): нет мусора/данных в git; `CLAUDE.md ↔ AGENTS.md`, отчёты,
  README/docs/configs, вшитый код `notebook_internal.ipynb` и экспорты пакета согласованы.
- Любое изменение всегда оформляется коммитом и pull request'ом в отдельной ветке (не в
  `main`); см. `docs/WORKFLOW.md` §«Git: коммиты и PR».
