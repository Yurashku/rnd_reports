...
- **Отчётные RnD** — каждая директория `rnd/NN_<topic>/` остаётся **report-oriented** и содержит **ровно** четыре файла (однородно для всех RnD):
  ...
  - `README.md` — короткое описание RnD.
...
Карта репозитория и правила для агентов — в [`CLAUDE.md`](CLAUDE.md) и синхронизированном [`AGENTS.md`](AGENTS.md).

## Структура репозитория

```
rnd/0N_<topic>/      # отчётные RnD — только {notebook.ipynb, report.md, report.pdf, README.md}
...
- `embeddings/` — тулкит-адаптеры RnD-7 для pyspark-эмбеддингов (`epk_id, report_dt, col_*`): `reducer.py` (`EmbeddingReducer`, PCA-снижение до `reducted_shape`), `propensity.py` (`PropensityScorer`, джойн с тритом + LogisticRegression → `propensity_score`); оба с in-time safety (обучение на `report_dt <= cutoff`), `experiment.py` — заглушки самого RnD. pyspark — **опциональный extra** (`pip install -e .[spark]`); импорт пакета его не требует.
...
- `configs/07_embedding_adjustment_set/` — дефолты адаптеров RnD-7 (`reducted_shape`, propensity, in-time cutoff).
...
- Внутри `rnd/0N_<topic>/` нет внешних `png/csv/txt`: компактные коммитимые артефакты (CSV-сводки, figures) живут в `results/0N_<topic>/`, графики встраиваются в `report.pdf`.
