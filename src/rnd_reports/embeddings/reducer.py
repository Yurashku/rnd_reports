...
class EmbeddingReducer:
    ...
    Параметры:
        ...
        reducted_shape: deprecated-alias к ``red_size`` (старое имя до переименования
            схемы R&D-7) — задаёт ``red_size`` с предупреждением ``DeprecationWarning``.

    Использование: ``fit(df, cutoff)`` → ``transform(df)`` (либо ``fit_transform``).
    """

    def __init__(
        self, red_size: int = 5, seed: int = 42, reducted_shape: int | None = None
    ) -> None:
        # Обратная совместимость: старое имя параметра ``reducted_shape`` (до переименования
        # схемы R&D-7 в ``red_size``/``red_*``). Поддерживаем как deprecated-alias, чтобы не
        # ломать ранее написанные ноутбуки/скрипты.
        if reducted_shape is not None:
            warnings.warn(
                "Параметр 'reducted_shape' переименован в 'red_size' и будет удалён; "
                "используйте red_size.",
                DeprecationWarning,
                stacklevel=2,
            )
            red_size = reducted_shape
        if red_size < 1:
    ...
    @property
    def reducted_shape(self) -> int:
        """Deprecated-alias к :attr:`red_size` (старое имя до переименования схемы R&D-7)."""
        warnings.warn(
            "Атрибут 'reducted_shape' переименован в 'red_size'; используйте red_size.",
            DeprecationWarning,
            stacklevel=2,
        )
