"""Сборка результатов бенчмарка R&D-6 в таблицу и график.

Превращает список ``MethodResult`` в ``pandas.DataFrame`` в порядке колонок
``RESULT_COLUMNS`` (§10 контекста), форматирует числовые поля и строит обязательный
график ATE ± доверительный интервал по методам (``plot_ate_ci``, §11 контекста).
"""
