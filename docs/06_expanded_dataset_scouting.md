...
| dunnhumby Complete Journey | github.com/bradleyboehmke/completejourney | yes | research_sandbox (observational) | coupon (не random) | redemption/spend | yes (rich) | yes | yes (engineered) | no | sandbox-construct | yes (campaign/coupon/contact) | yes (redemption/post-coupon) | C_D_sandbox | adapter (demo-only) | богатейший timing; observational → demo-only |
| CriteoPrivateAd | huggingface.co/datasets/criteo/CriteoPrivateAd | yes (sample) | event_log_sandbox (ad logs) | display/request | click/sale (delayed) | partial | yes | yes (pre-display) | no | no | yes (display/privacy) | yes (delayed report) | D_F_sandbox | adapter (demo-only) | D/F-полигон; не safe B/C |
...
- **Source / access:** R-пакет `bradleyboehmke/completejourney` (`get_transactions()` /
  `get_promotions()` + .rda-таблицы) либо dunnhumby.com source-files (регистрация).
- **Local load:** **yes** — собран в `dunnhumby/` (конвертация .rds/.rda → CSV через pyreadr):
  transactions 1 469 307×11 (реальный `transaction_timestamp` 2017), promotions 20 940 529×5,
  + campaigns / campaign_descriptions (`start_date`/`end_date`) / coupons / coupon_redemptions
  (`redemption_date`) / demographics / products. Schema: `dunnhumby/schema_summary.txt`.
- **Mapping (анкер = `campaign_descriptions.start_date`):** **A** = пред-анкерная история
  покупок + демография; **C (sandbox-construct)** = ранняя in-window активность
  `[start, start+cutoff)` строго до окна исхода; **D** = интенсивность таргетинга
  (n_prior_campaigns); **E** = redemptions внутри окна; **target** = spend
  `[start+cutoff, start+horizon)`.
- **Timing:** богатейший event-timing среди кандидатов, **но** observational (купоны не random).
- **Adapt:** **реализован** `load_completejourney_research_dataset` → ResearchDataset
  (`research_sandbox`); household-level, выбор анкера по max-охвату c достаточным pre/post-окном
  (по умолчанию campaign 18, 2017-10-30, 2469 hh, 49 A + 3 C + 1 D + 1 E). **Delta:** не входит в
  реальную A-only таблицу (observational, не RCT) — только для C/D-диагностики.
...
- **Source / access:** HuggingFace `criteo/CriteoPrivateAd`; `pip install datasets` (в .venv);
  288 parquet-шардов, партиции `day_int=1..30` (~100M строк).
- **Local load:** **yes (sample)** — stream-сэмпл 10 000×150 сохранён как
  `criteo_private_ad/sample_10k.parquet` (+ `event_per_user_correction.csv`, README, один шард);
  schema: `criteo_private_ad/schema_summary.txt`. Полный датасет не качается.
- **Schema (по паркету):** `id`/`user_id`, `display_order`, `campaign_id`, `publisher_id`;
  метки исхода `is_visit`/`is_clicked`/`is_click_landed`/`nb_sales` + `*_delay_after_display_array`;
  feature-бакеты: `features_ctx_*`, `features_browser_*`, `features_kv_*constrained_*`,
  и 80 плейсхолдеров `features_not_available_*`.
- **Mapping:** **A** = `features_ctx_*` / `features_browser_*` (контекст до показа); **D** =
  `features_kv_*constrained_*` / `campaign_id` / `publisher_id`; **F** = метки исхода и
  delay-counts; защитимый in-time C — нет (`day_int` в stream-сэмпле отсутствует).
- **Adapt:** **реализован** `load_criteo_private_ad_research_dataset` → ResearchDataset
  (`event_log_sandbox`); демо-treatment `display_order==1` (топ-показ), target `is_clicked`,
  18 A + 40 D + 6 F. **Delta:** нет A/B-контракта — только D/F-диагностика.
